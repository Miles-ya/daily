...
- 只抓取、解析和展示最近 90 天内发布的政策；更早或无法确认发布日期的文件会自动移出数据集。
- AI 队列按发布日期从新到旧处理，优先解析刚发布的政策。
- 网站只展示政策事实、公共解析、证据和不确定性。
- Telegram 才展示“对我意味着什么”“所以我该干什么”等个性化判断。
- 私人画像分为长期决策坐标系和按月更新的当前状态，只通过 GitHub Secret 注入。
- Telegram 将国家重要度与个人相关度分开：高相关且可行动才强提醒，低相关只发短信号，完全无关保持静默。
